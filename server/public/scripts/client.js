...
function updateDOM(tasksArray) {
  ...
  for (var i = 0; i < tasksArray.length; i++) {
    ...
    $row.append('<td><span class="delete glyphicon glyphicon-remove" role="button" data-id=' + taskID + '></span></td>');
...
function addEventListeners() {
  ...
  // '.update' class click handler - responds to 'upDate' buttons & checkboxes
  $('tbody').on('click', '.update', function() {
    var taskID = $(this).data('id');
    console.log('make a change for task', taskID);
    updateTask(taskID);
  }); // end click listener for '.update' class
